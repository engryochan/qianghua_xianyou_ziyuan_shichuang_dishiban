...
新增 `Watch-LocalDiagnostics.ps1`，默认持续运行，Ctrl+C 停止。显示本机系统、键盘设备和已建立 TCP 连接及本机进程。对端设备、系统及操作者无法通过这些资料确认，明确显示未知，不将普通连接认定为远控。

```powershell
& '.\诊断电脑模板\Watch-LocalDiagnostics.ps1' # 持续刷新本机可验证信息，默认不保存日志。
& '.\诊断电脑模板\Watch-LocalDiagnostics.ps1' -SaveChanges # 可选保存变化快照，单次运行最多十份、每份1MiB。
```

每轮采集后默认等待五秒，系统和键盘清单每分钟刷新；它是定期快照，可能遗漏短暂连接，不是完整网络取证。最多采集2000条连接、默认展示20条；`-DisplayRows` 可调整展示数。`-Iterations 2 -NoClear` 用于有限验收。旧运行目录不会自动删除。权限失败将明确显示。进程名不等于数字签名验证，连接方向、远端型号与系统也不能据此认定。

本机两个PowerShell运行时及沙箱外的两轮运行测试已通过；尚未进行长时间负载、人工Ctrl+C或故障复现验收，不能保证所有场景零性能影响。原有五个脚本保持不变。
