...
本機用於資料分析（R + Python + Parquet/DuckDB）。近期完成環境整備後，以實測方式盤點全機軟硬體，發現以下六項需要貴部門權限或決策才能處理。
...
## 申請五：Microsoft Edge 的更新機制失效（資安項目）

### 現象

Edge 版本停留在 **150.0.4078.105**（檔案日期 2026-07-30），目前最新為 **153.0.4234.48**，落後三個主要版本。

### 已查證的根因

| 檢查項目 | 結果 |
|---|---|
| 更新程式檔案 | **存在**：`C:\Program Files (x86)\Microsoft\EdgeUpdate\1.3.251.5\MicrosoftEdgeUpdate.exe` |
| `edgeupdate` / `edgeupdatem` 服務 | **不存在**（全機唯一含 edge 的服務只有 `MicrosoftEdgeElevationService`） |
| Edge 相關排程工作 | **不存在** |
| 群組原則 `HKLM:\SOFTWARE\Policies\Microsoft\EdgeUpdate` | 無 |
| 群組原則 `HKLM:\SOFTWARE\Policies\Microsoft\Edge` | 無 |

**更新程式本體還在，但呼叫它的服務與排程工作都不見了**，且沒有任何群組原則封鎖。因此：

- `edge://settings/help` 無法觸發更新（沒有更新服務可呼叫）
- `winget upgrade` 回報 `-1978335090`「安裝技術與當前安裝的版本不同，請先卸載」——Edge 屬機器層級安裝、由自家更新程式管理，winget 動不了

### 懇請協助

重新註冊 Edge 的更新服務與排程工作，或以貴部門的標準方式派送 Edge 更新。

**我們未嘗試自行修復**——重新註冊服務需猜測未公開的命令列參數，在公司資產上不宜由使用者自行嘗試。
...
使用者帳號 `PPCCpcpc` 本身即在本機 Administrators 群組內。**2026-09-21 09:00–09:08 期間，經使用者本人在 UAC 對話框逐次按下「是」後，執行了以下提權作業**：

| 時間 | 作業 | 結果 |
|---|---|---|
| 09:00 | `Run_AsAdmin.ps1 -All -WhatIf`（預演，不變更） | 僅列出計畫 |
| 09:03 | `Run_AsAdmin.ps1 -All`（實際執行） | 見下 |
| 09:07 | 唯讀驗證（還原點、Defender 排除、陰影複製） | 僅讀取 |

09:03 實際變更四項：

| 變更 | 內容 | 還原方式 |
|---|---|---|
| 長路徑 | `LongPathsEnabled` 0 → 1 | 改回 0 |
| 分頁檔 | 自動管理 → 固定 16290／32581 MB | 備份於 `~\pagefile_backup_20260921_090322.txt`；系統內容 > 進階 > 虛擬記憶體 > 自動管理 |
| Defender 排除 | 新增 `C:\work`、`...\R\win-library`、`...\uv` 三項 | `Remove-MpPreference -ExclusionPath` |
| VCRedist x86 | 14.42 → 14.51.36247.0（winget 官方來源） | winget 可降版 |

系統還原點嘗試建立但**未成功**（Windows 24 小時節流）。既有還原點 `#2 2026-09-20 17:54「Windows 更新」`，時間早於上述所有變更，仍可作為退路。

完整逐行記錄：`~\Run_AsAdmin_20260921_090017.log`（預演）、`~\Run_AsAdmin_20260921_090322.log`（實際執行），為 PowerShell Transcript，可與資安日誌逐條對照。

**上述提權作業皆為本機管理員權限範圍內的設定調整，未觸及任何資安軟體、群組原則、服務或韌體。**
